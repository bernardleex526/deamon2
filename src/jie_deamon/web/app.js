...
let lidarData = {
    ...
    m20Preview: false,         // NEW-06: 仅算法/NavCmd预览模式
    targetSelected: false,     // NEW-06: 目标已人工选点
    trackingValid: false       // NEW-06: 目标跟踪有效（丢失=stale）
};
...
function initTabSwitching() {
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            // 发送模式切换指令
            // NEW-06: M20预览模式下仅允许跟随面板，直控/导航面板禁止进入
            if (lidarData.m20Preview && tab.dataset.tab !== 'follow') {
                console.warn('M20预览模式: 仅算法/NavCmd预览，直控/导航已禁用（机器不会运动）');
                return;
            }
...
function handleData(data) {
    if (data.type === 'scan_data') {
        ...
        lidarData.m20Preview = data.m20_preview || false;
        lidarData.targetSelected = data.is_selected || false;
        lidarData.trackingValid = data.is_tracking_valid || false;

        updateUI();
    }
}

// NEW-06: 预览模式横幅与禁用面板（随m20_preview状态动态展示）
function updatePreviewBanner() {
    const banner = document.getElementById('previewBanner');
    if (banner) {
        banner.style.display = lidarData.m20Preview ? 'block' : 'none';
    }
    // M20预览: 当前停在直控/导航面板时强制切回跟随面板（两者已禁用）
    if (lidarData.m20Preview) {
        const activeTab = document.querySelector('.nav-item.active');
        if (activeTab && activeTab.dataset.tab !== 'follow') {
            const followTab = document.querySelector('.nav-item[data-tab="follow"]');
            if (followTab) followTab.click();
        }
    }
}

// NEW-06: 目标选点/跟踪状态标记（target stale可见）
function updateTargetSelectState() {
    const el = document.getElementById('targetSelectState');
    if (!el) return;
    if (!lidarData.targetSelected) {
        el.textContent = '未选点（双击雷达图选点）';
        el.style.color = '#9ca3af';
    } else if (lidarData.trackingValid) {
        el.textContent = '已选点 · 跟踪中';
        el.style.color = '#10b981';
    } else {
        el.textContent = '已选点 · 目标丢失(需重新选点+使能)';
...
function initControls() {
    ...
    if (btnLieDown) {
        btnLieDown.addEventListener('click', () => {
            // NEW-06: M20预览模式禁用D1动作按钮（后端同样拒绝）
            if (lidarData.m20Preview) {
                console.warn('M20预览模式: D1动作已禁用（机器不会运动）');
                return;
            }
